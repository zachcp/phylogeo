...
---
`r library("knitr")`
`r opts_chunk$set(cache=FALSE, fig.width=15, message=FALSE, warning=FALSE)`
...
### map_tree_kmeans
*map_tree_kmenas* provides a way to explore the spatial distribution of subsets of your data. From your phylogentic tree (required), a distance matrix is used to make a kmenas calculation which will cluster your tree into *k* number of clusters. The members of these clusters will then be displayed on the map.
```{r}
#clustered trees plus maps
map_tree_kmeans(epoxamicin_KS, clusternum=6)
```

## phyloseq plotting functions

### plot_greatcircle_distance
