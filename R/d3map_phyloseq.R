...
d3map_phyloseq <- function(physeq, tax_column, mapdata="USA", omitNA=TRUE){
    ...
    possible_maps = c("USA","World50m", "World110m")
    ...
    servedata(jsondata = toJSON(samdata),
              mapfile = mapfile,
              mapjsfile = mapJS,
              basecss = basecss,
              htmlfile = deps)
...
servedata <- function(mapfile,
    ...
    rook_handler <- function(env){
        ...
        if(http_method == "GET"){
            ...
            list (
                status = 200,
                body = paste(includeHTML(htmlfile),
                             includeScript(mapfile),
                             paste0("<script>var pointdata = ", jsondata ,"</script>"),
                             includeScript(mapjsfile),
                             includeCSS(basecss),
