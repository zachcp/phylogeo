...
#' and special care should be given to considering the choice of this threshold.
#'
#' @usage draw_network(physeq, type="samples", maxdist=0.9, distance="jaccard"
#'   color=NULL, shape=NULL, region=NULL, point_size=4, alpha=1,
#'   label="value", hjust = 1.35, 
#' 	line_weight=0.5, line_color=color, line_alpha=0.4,
#' 	layout.method=layout.fruchterman.reingold, title=NULL)
map_network <- function(physeq, maxdist=0.9, distance="jaccard" ,sampledata, color=NULL, region=NULL, pointsize=4, pointalpha = 0.8){
  .check_physeq(physeq)
  ...
  networkplot <- plot_network(ig, ad35, color=j)
  ...
  data <- merge(sampledata, )
  
  
      ggsave(network_AD, filename=paste('../../presentations/Rpresentations/Pictures/ad35_network_',j,'_',as.character(i),'.png',sep=""),
             width=8,height=6)
    }}
