...
is simple a continuation of that tutorial. We create some random data from phyloseq including the following: 

- an OTU table, 
- a Taxonomy Table, and a 
- sample information dataframe.
- Latitude and Longitude information to ass to the sample data, and
- A phylogenetic tree of the data.

The last two items are neccessary to take advantage of phylogeo's plotting utilities.


### Load phylogeo and get some sequecne data from the `ape` package
Sequence data from the [ape](http://ape-package.ird.fr/) package will be used as an example, although you will probably have your own sequnecing data to use. 
```{r, message = FALSE}
#load libraries
library(phyloseq)
library(phylogeo)
